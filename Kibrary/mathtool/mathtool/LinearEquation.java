...
public class LinearEquation {
	...
	private Complex[] TwoDimensionEquation() {
		double a = 1;
		double b = this.coef[1] / this.coef[2];
		double c = this.coef[0] / this.coef[2];
		...
		if (D == 0) {
			...
			D = Math.sqrt(-D);
			return new Complex[] { Complex.valueOf(-b / 2, -D / 2),
					Complex.valueOf(-b / 2, D / 2) };
		} else {
			D = Math.sqrt(D);
			return new Complex[] { Complex.valueOf((-b - D) / 2),
					Complex.valueOf((-b + D) / 2) };
		}
	}
	...
	private Complex[] ThreeDimensionEquation() {
		...
		// }
		// double a = 1;
		double b = this.coef[2] / this.coef[3];
		double c = this.coef[1] / this.coef[3];
		double d = this.coef[0] / this.coef[3];
		// System.out.println("Calculating "+a+"x^3 + "+b+"x^2 + "+c+"x + "+d+"
		// =0");
		Complex omega = new Complex(-0.5, Math.sqrt(3) / 2);
		Complex omega2 = omega.pow(new Complex(2, 0));
		double q = c - b * b / 3D;
		double r = d - b * c / 3D + 2 * b * b * b / 27D;
		...
		if (rr == 0)
			...
			sr = zr.pow(new Complex(0.5, 0));
		if (r > 0 && sr.abs() != 0)
			sr = sr.multiply(new Complex(-1, 0));
		sr = sr.add(new Complex(-r / 2, 0));
		Complex u;
		if (sr.abs() == 0)
			u = new Complex(0, 0);
		else
			u = sr.pow(new Complex(1 / 3D, 0));
		if (u.abs() == 0)
			return new Complex[] { Complex.valueOf(-b / 3) };
		Complex v = new Complex(-q / 3, 0);
		v = v.divide(u);
		Complex[] x = new Complex[3];
		Complex b3 = new Complex(b / 3, 0);
		...
		Arrays.sort(x, (o1,o2)->{
			return (int) (Math.abs(((Complex) o1).getImaginary()) - Math
					.abs(((Complex) o2).getImaginary()));
		});
	...
	private int Discriminant3() {
		...
		// "+b+"x^2 + "+c+"x + "+d+" =0");
		double D = b * b * c * c + 18 * a * b * c * d - 4 * a * c * c * c - 4
				* b * b * b * d - 27 * a * a * d * d;
		if (D > 0)
			return 31; // 3 Real
		if (D < 0)
