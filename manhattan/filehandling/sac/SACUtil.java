...
public final class SACUtil {
	 ...
	 * 
	 * @param sacEPath
	 *            SAC file which component is E. must exist.
	 * @param sacNPath
	 *            SAC file which component is N. must exist.
	 * @param outputRPath
	 *            for output SAC with respect to R
	 * @param outputTPath
	 *            for output SAC with respect to T
	...
	public static boolean rotate(Path sacEPath, Path sacNPath, Path outputRPath, Path outputTPath) throws IOException {
		if (Files.exists(outputRPath))
			throw new FileAlreadyExistsException(outputRPath.toString());
		if (Files.exists(outputTPath))
			throw new FileAlreadyExistsException(outputTPath.toString());
		...
		if (npts != Integer.parseInt(mapN.get(SACHeaderEnum.NPTS)))
			return false;
	 ...
	 * 
	 * @param outPath
	 *            for output (If the file exists, it will be overwritten)
	 * @param headerMap
	 *            of output SAC
	 * @param data
	 *            of output SAC. The length must be same as NPTS.
	 * @param options
	 *            Options for output
	 * @throws IOException
	 ...
	 * 
	 * @param sacPath
	 *            to extract waveform from
	 * @return DEEP copied {@link Trace} from waveform in the sacFile
	 * @throws IOException
	 *             If sacFile does not exist, if an I/O error occurs.
	 */
