...
//		List<PerturbationPoint> checkerboard = checkerboardDppCACAR_100km(2.);
		List<PerturbationPoint> checkerboard = anyStructure(3.);
		Path outpath = Paths.get("/Users/YUKI/Desktop/checkerboard_2per.inf");
		try {
...
//							, layerSign * lonSign * latSign * dvs);
	...
	public static List<PerturbationPoint> checkerboardDppCACAR_100km(double dvs) {
		...
		for (int k = 0; k <= 4;k++) {
			double r = 3880 - k * 100;
			double lonSign = 1;
			for (int i = -178; i <= 177; i+=5) {
				double latSign = 1;
				for (int j = -88; j <= 87; j+=5) {
					Location location = new Location(j+.5, i+.5, r);
					PerturbationPoint perturbation = new PerturbationPoint(location
							, layerSign * lonSign * latSign * dvs);
					perturbations.add(perturbation);
					latSign *= -1;
