...
public class SACFileName extends File {
    ...
    public static boolean isSYN(String fileName) {
        if (!isOBSorSYN(fileName)) return false;
    ...
    public static boolean isSacFileName(File file) {
        ...
        if (part.length != 7 && part.length != 3) return false;

        // station name must be 8 letters or shorter.
        if (8 < part[0].length()) return false;

        // global cmt idかどうか
        if (!GlobalCMTID.isGlobalCMTID(part[1])) return false;
    ...
    private void readName(String fileName) {
        if (!isSacFileName(fileName)) throw new IllegalArgumentException(fileName + " is an invalid sac file name");
