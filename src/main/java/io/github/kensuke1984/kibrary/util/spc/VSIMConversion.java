 ...
 *
 */
public final class VSIMConversion {
	 ...
	 *            structure
	 */
	public VSIMConversion(PolynomialStructure structure) {
		...
		for (int i = 0; i < spectrum.nbody(); i++) {
			...
			for (int ip = 0; ip < np + 1; ip++) {
				...
				for (int iComponent = 0; iComponent < body.getNumberOfComponent(); iComponent++) {
					...
					uIm[iComponent] = u.multiply(fact).multiply(Complex.I);
				}
				newBody.add(ip, uIm);
			}
