...
public class SshDSMInformationFileMaker implements Operation {
    ...
    public static void writeDefaultPropertiesFile() throws IOException {
        Path outPath =
                Paths.get(SshDSMInformationFileMaker.class.getName() + Utilities.getTemporaryString() + ".properties");
    ...
    private void set() {
        ...
        perturbationR =
                Arrays.stream(property.getProperty("perturbationR").split("\\s+")).mapToDouble(Double::parseDouble)
                        .toArray();
    }
    ...
    public void run() throws Exception {
        ...
        for (EventFolder eventDir : eventDirs) {
            ...
            if (numberOfStation != stations.size()) System.err.println(
                    "!Caution there are stations with the same name and different positions in " +
                            eventDir.getGlobalCMTID());
            SshDSMinfo info =
                    new SshDSMinfo(ps, eventDir.getGlobalCMTID().getEvent(), stations, header, perturbationR, tlen, np);
