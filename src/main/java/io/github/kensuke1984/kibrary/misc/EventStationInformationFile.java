 ...
 *
 */
public final class EventStationInformationFile {
	...
	private EventStationInformationFile() {
	}
	...
	public static void createStationInformationFile(Path workPath, OpenOption... options) throws IOException {
		...
		Set<Station> stationSet = sacNameSet.stream().filter(s -> s.getComponent().equals(SACComponent.T)).filter(s -> s.isOBS()).map(sacName -> {
//		Set<Station> stationSet = sacNameSet.stream().map(sacName -> {
			try {
				System.out.println(sacName.readHeader().getStation().getName()+ " "
						+ sacName.readHeader().getStation().getNetwork()+ " "
						+ sacName.readHeader().getStation().getPosition().toLocation(0) + " "
						+ sacName.readHeader().getGlobalCMTID() + " "
						+ sacName.readHeader().getEventLocation() + " "
						+ Math.toDegrees(sacName.readHeader().getEventLocation().getEpicentralDistance(sacName.readHeader().getStation().getPosition())) + " "
						+ Math.toDegrees(sacName.readHeader().getEventLocation().getAzimuth(sacName.readHeader().getStation().getPosition()))
						);
