...
public class TimewindowMaker implements Operation {
    ...
    private static Timewindow[] mergeWindow(Timewindow[] windows) {
        ...
        for (int i = 1; i < windows.length; i++) {
            ...
            if (windowA.overlap(windowB)) {
                ...
                if (i == windows.length - 1) windowList.add(windowA);
            } else {
                windowList.add(windowA);
                windowA = windows[i];
                if (i == windows.length - 1) windowList.add(windows[i]);
            }
        }
    ...
    private void checkAndPutDefaults() {
        ...
        if (!property.containsKey("exPhases")) property.setProperty("exPhases", "sS");
    ...
    private void makeTimeWindow(SACFileName sacFileName) throws IOException {
        ...
        Arrays.stream(windows).map(window -> fix(window, delta)).filter(window -> window.endTime <= e)
                .map(window -> new TimewindowInformation(window.getStartTime(), window.getEndTime(), station, id,
                        component)).forEach(timewindowSet::add);
    }
    ...
    private Timewindow[] createTimeWindows(double[] phaseTime, double[] exPhaseTime) {
        Timewindow[] windows =
                Arrays.stream(phaseTime).mapToObj(time -> new Timewindow(time - frontShift, time + rearShift)).sorted()
        ...
        if (exWindows == null) return windows;
