    ...
    public Collection<Intersection> execute(List<LineSegment> lineSegments) {
        ...
        for (LineSegment lineSegment : lineSegments) {
            ...
            if (lineSegment.getPointA().y < lineSegment.getPointB().y ||
                ...
                eventQueue
                        .add(new Event(SegmentType.SEGMENT_START, lineSegment.getPointA().x, lineSegment.getPointA().y,
                                lineSegment, null));
                eventQueue.add(new Event(SegmentType.SEGMENT_END, lineSegment.getPointB().x, lineSegment.getPointB().y,
                        lineSegment, null));
            } else {
                eventQueue
                        .add(new Event(SegmentType.SEGMENT_START, lineSegment.getPointB().x, lineSegment.getPointB().y,
                                lineSegment, null));
        ...
        while ((event = eventQueue.pollFirst()) != null) {
            ...
            switch (event.type) {
                ...
                case INTERSECTION: // 交点イベントの場合
                    ...
                    if (sweepComparator.compare(left, right) < 0) sweepComparator.setY(sweepY + 0.001);
