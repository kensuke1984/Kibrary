...
public class ConvexPolygon {
     ...
     *
     * @param vertices {@link Point2D}s for this. They must be in order and they must
    ...
    public ConvexPolygon(Point2D... vertices) {
        ...
        for (int i = 1; i < size; i++) {
            ...
            if (ccw0 * ccw == 0) throw new IllegalArgumentException("Polygon may not need " + v2);
        }
    ...
    public boolean contains(Point2D point) {
        ...
        for (LineSegment edge : edges)
            ...
            else if (edge.intersects(halfLine)) count++;
