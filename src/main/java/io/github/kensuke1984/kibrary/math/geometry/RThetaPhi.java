...
public class RThetaPhi {
    ...
    public RThetaPhi(double r, double theta, double phi) {
        if (!checkValidity(r, theta, phi)) throw new IllegalArgumentException();
        this.r = r;
        this.theta = theta;
        this.phi = phi;
        fixPhi();
    ...
    private void fixPhi() {
        ...
        while (2 * Math.PI <= phi) phi -= 2 * Math.PI;
        if (phi < 0 || 2 * Math.PI <= phi) System.err.println(phi + "(" + Math.toDegrees(phi) + "deg) is used");
    }
