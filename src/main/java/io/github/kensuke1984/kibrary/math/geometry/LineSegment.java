...
class LineSegment {
	...
	public boolean contains(Point2D point) {
		if (point.x < Double.min(pointA.x, pointB.x) || Double.max(pointA.x, pointB.x) < point.x)
			return false;
		if (point.y < Double.min(pointA.y, pointB.y) || Double.max(pointA.y, pointB.y) < point.y)
			return false;
