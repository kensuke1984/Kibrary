...
public class RaypathDistribution implements Operation {
    ...
    private void set() throws IOException {
        ...
        if (property.containsKey("timeWindowInformationPath")) {
            ...
            if (Files.exists(f)) timeWindowInformationFile = TimewindowInformationFile.read(f);
        }
    }
    ...
    private void outputStation() throws IOException {
        List<String> lines =
                stationSet.stream().map(station -> station + " " + station.getPosition()).collect(Collectors.toList());
        if (!lines.isEmpty()) Files.write(stationPath, lines);
    }
    ...
    private void outputRaypath() throws IOException {
        List<String> lines = Utilities.eventFolderSet(workPath).stream().flatMap(eventDir -> {
            try {
                ...
                }).filter(Objects::nonNull).map(header -> header.getSACString(SACHeaderEnum.KSTNM) + " " +
                        header.getSACString(SACHeaderEnum.KEVNM) + " " + header.getEventLocation() + " " +
                        Station.of(header).getPosition()).collect(Collectors.toList());
    ...
    private GMTMap createBaseMap() {
        ...
        double minimumEventLatitude =
                ids.stream().mapToDouble(id -> id.getEvent().getCmtLocation().getLatitude()).min().getAsDouble();
        double maximumEventLatitude =
                ids.stream().mapToDouble(id -> id.getEvent().getCmtLocation().getLatitude()).max().getAsDouble();
        ...
        double minimumStationLatitude =
                stationSet.stream().mapToDouble(s -> s.getPosition().getLatitude()).min().orElse(minimumEventLatitude);
        double maximumStationLatitude =
                stationSet.stream().mapToDouble(s -> s.getPosition().getLatitude()).max().orElse(maximumEventLatitude);

        double minimumStationLongitude =
                stationSet.stream().mapToDouble(s -> s.getPosition().getLongitude()).map(d -> 0 <= d ? d : d + 360)
                        .min().orElse(minimumEventLongitude);
        double maximumStationLongitude =
                stationSet.stream().mapToDouble(s -> s.getPosition().getLongitude()).map(d -> 0 <= d ? d : d + 360)
                        .max().orElse(maximumEventLongitude);
        ...
        int maxLatitude = (int) Math
                ...
                5 * 5 + 10;
        int minLongitude = (int) Math.round(minimumEventLongitude < minimumStationLongitude ? minimumEventLongitude :
                minimumStationLongitude) / 5 * 5 - 10;
        int maxLongitude = (int) Math.round(maximumEventLongitude < maximumStationLongitude ? maximumStationLongitude :
                maximumEventLongitude) / 5 * 5 + 10;
        if (minLatitude < -90) minLatitude = -90;
        if (90 < maxLatitude) maxLatitude = 90;
