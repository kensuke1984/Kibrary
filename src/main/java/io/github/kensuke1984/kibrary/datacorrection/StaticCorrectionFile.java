...
public final class StaticCorrectionFile {
    ...
    public static Set<StaticCorrection> read(Path infoPath) throws IOException {
        try (DataInputStream dis = new DataInputStream(new BufferedInputStream(Files.newInputStream(infoPath)))) {
            ...
            for (int i = 0; i < nInfo; i++)
                dis.read(bytes[i]);
            Set<StaticCorrection> staticCorrectionSet =
                    Arrays.stream(bytes).parallel().map(b -> createCorrection(b, stations, cmtIDs))
    ...
    public static void write(Path outPath, Set<StaticCorrection> correctionSet, OpenOption... options)
        ...
        Map<Station, Integer> stationMap =
                IntStream.range(0, stations.length).boxed().collect(Collectors.toMap(i -> stations[i], i -> i));
        Map<GlobalCMTID, Integer> idMap =
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length != 0) scf = read(Paths.get(args[0]));
        else {
