...
public final class StationInformationFile {
    ...
    public static Set<Station> read(Path infoPath) throws IOException {
        ...
        try (BufferedReader br = Files.newBufferedReader(infoPath)) {
            br.lines().map(String::trim).filter(line -> !line.startsWith("#")).forEach(line -> {
                ...
                if (!stationSet.add(st)) throw new RuntimeException("There is duplication in " + infoPath);
            });
        }
    ...
    public static void main(String[] args) throws IOException {
        if (0 < args.length) {
            ...
            if (Files.exists(f) && Files.isDirectory(f)) createStationInformationFile(f);
            else System.err.println(f + " does not exist or is not a directory.");
        } else {
            Path workPath;
            String path = "";
            do {
