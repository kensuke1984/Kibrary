...
final class RaypathPanel extends JPanel {
    ...
    void toEPS(OutputStream outputStream, Phase phase, double rayparameter, double delta, double time, double eventR)
        ...
        EpsGraphics epsGraphics =
                new EpsGraphics(phase.toString(), outputStream, 0, 0, getWidth(), getHeight(), ColorMode.COLOR_RGB);
        ...
        String line =
                phase + ", Ray parameter: " + rayparameter + ", Depth[km]:" + depth + ", Epicentral distance[deg]: " +
                        delta + ", Travel time[s]: " + time;
    ...
    synchronized void addPath(double[] x, double[] y) {
        if (x.length != y.length) throw new IllegalArgumentException("Unexpected");
